...
export class ConsultingFormComponent implements OnInit {
  ...
  doctors = [];
  specialities = [];
  constructor(private fb:FormBuilder , private _homepageService:HomepageService , private _snackbar:MatSnackBar , private _abhaService:AbhaCardService ) { }

  ngOnInit(): void {
    this._homepageService.getSpecialities().subscribe((data:[])=>{
  ...
  onSubmit(){
    ...
    this._homepageService.saveConsultion(counseltFormValues).subscribe((data)=>{
      this._snackbar.open(" consultion request is created", "Okay" ,{duration:3000 })
  ...
  fetchDoctor(event:MatSelectChange){
    ...
    this._homepageService.getDoctors(event.value).subscribe((doctorData:[])=>{
